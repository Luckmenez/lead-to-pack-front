...
const config: Record<ProfileType, ProfileConfig> = {
  comprador: {
    title: "Comprador",
    description:
  ...
  profissional: {
    ...
    description: "Para profissionais independentes do setor de embalagens",
    price: "R$ 000,00/mês",
    sub: "Cancele quando quiser",
    icon: BriefcaseIcon,
    color: "#1F2F44",
  ...
  return (
    <Card className="flex h-[420px] w-[280px] flex-col rounded-xl border shadow-sm transition hover:shadow-md">
      <CardContent className="flex flex-col items-center text-center gap-3 pt-7">
        ...
        <h3 className="text-base font-semibold" style={{ color: data.color }}>
          {data.title}
        </h3>

        <p className="px-5 text-sm text-muted-foreground">{data.description}</p>
