...
function ProfPaymentContent() {
  ...
  const closePortfolioWarning = useCallback(() => {
    ...
    router.replace(`/prof-registration/payment?${params.toString()}`)
        ...
        <div className="mx-auto max-w-md">
          ...
          <div className="mt-8 flex justify-center">
            <Button
              ...
              onClick={() =>
                router.push(
                  `/prof-registration/success?payment=${payment}`
                )
              }
            >
              Finalizar cadastro
...
export default function ProfPaymentPage() {
  return (
    <Suspense fallback={
      <main className="w-full">
        <div className="mx-auto max-w-6xl px-6 py-10">
          <ProgressBar step={2} />
          <div className="mb-6 flex justify-end">
            <Link href="/prof-registration" className="flex items-center gap-2 rounded-full bg-[#E7EFF5] px-4 py-1.5 text-sm font-medium text-[#4F83A6] hover:bg-[#dbe7f0]">
              <ArrowLeftIcon size={14} weight="bold" />
              Voltar
            </Link>
          </div>
