...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const user =
      rawUser && tipo ? ({ ...JSON.parse(rawUser), tipo } as AuthUser) : null;
  ...
  const loginComprador = useCallback(
    (token: string, comprador: CompradorUser) => {
  ...
  const loginFornecedor = useCallback(
    (token: string, fornecedor: FornecedorUser) => {
